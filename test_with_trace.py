...
def trace_log(direction: str, arbid: int, data: bytes):
    global TRACE_FILE, TRACE_T0

    if TRACE_FILE is None:
        return

    ts = time.time() - TRACE_T0
    TRACE_FILE.write(
        f"{ts:10.6f} CAN1 {arbid:03X} {direction} {len(data)} {hx(data)}\n"
    )
...
def recv(timeout: float = 2.0):
    ...
    while time.time() < end:
        ...
        # ✅ 所有帧记录
        trace_log("Rx", m.arbitration_id, data)
...
def run():
    ...
    TRACE_T0 = time.time()
    TRACE_FILE = open("python_trace.txt", "w", encoding="utf-8")
    TRACE_FILE.write("time chn id dir dlc data\n")
